...
import math # Keep math for quaternion_to_euler (though yaw isn't saved)
import os
import glob
...
class WaypointCollector:
    def __init__(self):
        ...
        try:
            ...
            self.file.write("# Format: x y\n") # MODIFIED: Indicate only X, Y
    ...
    def save_current_pose(self):
        """Saves the current X, Y pose (from the latest message) to the file."""
        if self.latest_pose is None:
        ...
        # orientation_q = pose_msg.orientation # Not needed anymore

        # Calculate yaw from the quaternion (Optional: could still calculate for logging if desired)
        # try:
        ...
        #      yaw = 0.0 # Default if conversion fails

        # Write X, Y to file
        try:
            self.file.write(f"{x:.3f} {y:.3f}\n") # MODIFIED: Write only X, Y
            self.file.flush() # Ensure data is written immediately
            self.num_waypoints += 1
            rospy.loginfo("[INFO] Waypoint saved: [x: %.2f, y: %.2f]", x, y) # MODIFIED: Log message
        except IOError as e:
    ...
    def quaternion_to_euler(self, x, y, z, w):
        ...
        # (This function remains the same, though not strictly needed for saving)
        t0 = +2.0 * (w * x + y * z)
        t1 = +1.0 - 2.0 * (x * x + y * y)
